...
from .comparison import (
    compare_loras, compare_parameters, format_resolution, image_resolution,
)
from .metadata_normalization import lora_signature as build_lora_signature
from .metadata_normalization import normalise_prompt
...
class ComparisonDialog(QDialog):
    ...
    def _populate_table(self) -> None:
        fields = compare_parameters(
            self.path_a, self.summary_a, self.path_b, self.summary_b
        )
        self.table.setRowCount(len(fields))
        difference_brush = QBrush(QColor(58, 79, 102))
        for row, field in enumerate(fields):
            values = (field.name, field.value_a or "—", field.value_b or "—")
            for column, value in enumerate(values):
...
def normalised_prompt(value: str) -> str:
    """Backward-compatible spelling used by existing similarity search code."""
    return normalise_prompt(value)


def lora_signature(metadata: dict[str, Any]) -> tuple[tuple[str, str, str], ...]:
    """Return a stable, order-independent LoRA signature."""
    return build_lora_signature(
        {
            "name": row.display_name,
            "model_strength": (row.value_a or row.value_b).model_strength,
            "clip_strength": (row.value_a or row.value_b).clip_strength,
        }
        for row in compare_loras(metadata, {})
    )
